...
const int8_t alpha_to[] PROGMEM = { 1,  2,  4,  8, 16,  5, 10, 20, 13, 26,
                           17,  7, 14, 28, 29, 31, 27, 19,  3,  6,
                           12, 24, 21, 15, 30, 25, 23, 11, 22,  9,
                           18, 0};

// Index of alpha^i:
const int8_t index_of[] PROGMEM = {-1,  0,  1, 18,  2,  5, 19, 11,  3, 29,
...
void CBCH3121::encode(uint32_t& data)
{
  ...
  uint32_t tmp = data;
  uint32_t gen_poly = ((uint64_t)POCSAG_BCH_GENPOLY << POCSAG_BCH_K);

  // Calculate BCH check bits
  for (unsigned int i = 0U; i < POCSAG_BCH_K; i++, tmp <<= 1) {
    //Serial.print("i="); Serial.print(i); Serial.print(" tmp="); Serial.println(tmp, HEX);
    if (tmp & 0x80000000U)
      tmp ^= gen_poly;
  }
...
bool CBCH3121::decode(uint32_t& data, uint16_t& errors)
{
  ...
  if (m_S1 == -1 && m_S3 == -1) {
    ...
    if (S1_3 == m_S3) {
      ...
      if (m_S3 != -1)
        tmp = pgm_read_byte(&alpha_to[S1_3]) ^ pgm_read_byte(&alpha_to[m_S3]);
      else
        tmp = pgm_read_byte(&alpha_to[S1_3]);

      X2 = (pgm_read_byte(&index_of[tmp]) - m_S1 + POCSAG_BCH_N) % POCSAG_BCH_N;
      ...
      for (uint8_t i = 1U; i <= POCSAG_BCH_N; i++) {
        ...
        if (X1 != -1) {
          ...
          Q ^= pgm_read_byte(&alpha_to[X1]);
        }

        if (X2 != -1) {
          X2 = (X2 + 2) % POCSAG_BCH_N;
          Q ^= pgm_read_byte(&alpha_to[X2]);
        }
...
void CBCH3121::calc_syndrome(uint32_t data)
{
  ...
  for (uint8_t i = 0; i < POCSAG_BCH_N; i++, data >>= 1) {
    if (data & 0x01U) {
      m_S1 ^= pgm_read_byte(&alpha_to[i % POCSAG_BCH_N]);
      m_S3 ^= pgm_read_byte(&alpha_to[(3 * i) % POCSAG_BCH_N]);
    }
  }

  // Transform polynomial form to index form
  m_S1 = pgm_read_byte(&index_of[m_S1]);
  m_S3 = pgm_read_byte(&index_of[m_S3]);
}
